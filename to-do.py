...
def see_tasks():
    if not to_do_list:
        ...
        print_border()
        max_length = max(len(task) for task in to_do_list)
        for index, task in enumerate(to_do_list, start=1): 
            print(f"| {index}. {task:<{max_length}} |")  # Padding the task to make things align
        print_border()
...
def main():
    ...
    while True:
        see_tasks()
        print("1 - Add, 2 - Move, 3 - Remove, 4 - Edit, 5 - Save, 6 - Exit")
